...
import { interpolateTrajectory, type TelemetryData, type Units } from '@/lib/types'
...
export default function MissionTracker({ missionSlug }: { missionSlug: string }) {
    const mission = getMission(missionSlug)!
    const { launchTime, splashdownTime, milestones, phases, crewActivities, crew } = mission

    const launchTs = Math.floor(launchTime.getTime() / 1000)
    const splashTs = Math.floor(splashdownTime.getTime() / 1000)
    const missionEnded = Date.now() / 1000 > splashTs

    const [data, setData] = useState<TelemetryData | null>(null)
    const [error, setError] = useState<string>('')
    const [isLive, setIsLive] = useState(!missionEnded)
    const [isPlaying, setIsPlaying] = useState(false)
    const [playSpeed, setPlaySpeed] = useState(720)
    const [displayTime, setDisplayTime] = useState(() =>
        missionEnded ? launchTs : Math.min(Date.now() / 1000, splashTs)
    )
    ...
    useEffect(() => {
        ...
        if (t) {
            ...
            if (!isNaN(metSeconds)) {
                setDisplayTime(launchTs + metSeconds)
                setIsLive(false)
            }
        }
    }, [launchTs])
    ...
    const fetchData = useCallback(async () => {
        try {
            const res = await fetch(`/api/telemetry?mission=${missionSlug}`)
    ...
    }, [isPlaying, isLive, playSpeed, splashTs])

    const handleDisplayTimeChange = useCallback((ts: number) => {
        setDisplayTime(Math.max(launchTs, Math.min(ts, splashTs)))
    ...
    const handleGoLive = useCallback(() => {
        if (missionEnded) {
            // For historical missions, "Go Live" goes to end of mission
            setDisplayTime(splashTs)
            setIsPlaying(false)
            setIsLive(false)
        } else {
            setIsLive(true)
    ...
    }, [missionEnded, splashTs])

    if (!data) {
        return (
            <div className="flex items-center justify-center h-screen">
                <div className="text-center">
                    <div className="text-xl mb-2">Loading {mission.name} telemetry...</div>
    ...
    const currentActivity = crewActivities.find(
    ...
    const missionComplete = metSeconds >= lastMilestone.metSeconds
    ...
    return (
        <div className="min-h-screen flex flex-col">
                ...
                <div className="w-full md:w-[340px] md:h-[650px] shrink-0 border-t md:border-t-0 md:border-l border-border bg-card flex flex-col overflow-hidden">
                    <div className="flex-1 md:overflow-y-auto">
                        <MissionStatus
                            ...
                            totalDays={mission.totalDays}
                            milestoneContext={mission.milestoneContext}
                            phaseDescriptions={mission.phaseDescriptions}
                            getNarrative={mission.getNarrative}
            ...
            <div className="max-w-[1400px] mx-auto w-full px-4 md:px-5 py-4 space-y-4">
                {viewMode === 'expert' && (
                    <TelemetryCharts trajectory={data.trajectory} displayTimestamp={displayTime} isLive={isLive && !missionEnded} metSeconds={metSeconds} />
                )}
                {mission.liveStreamUrl && (
                    <LiveStream url={mission.liveStreamUrl} title={mission.liveStreamTitle} />
                )}
